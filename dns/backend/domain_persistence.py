...
class DomainPersistenceManager:
    ...
    def process_final_dataset(
        ...
        #
        # Chosen structure: List[Dict[str, Optional[str]]], one dict per
        ...
        #     List[Dict[str, Optional[str]]] directly, or map each dict
        ...
        unknown_domain_records: List[Dict[str, Optional[str]]] = []
    ...
    def record_query_observation(
        ...
        """Atomically record repeated DNS query observation."""
        if not self.enable_persistence or not self._service:
            return False
        return self._service.record_query_observation(domain_name, observation_time)
    ...
    def update_external_review(
        ...
        """Record external intelligence check outcome."""
        if not self.enable_persistence or not self._service:
            return False
        return self._service.update_external_review(domain_name, new_status, review_time, metadata)
