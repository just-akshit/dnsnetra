...
# Canonical verdicts constant tuple
CANONICAL_VERDICTS = (
...
def normalize_verdict(value: Any) -> CanonicalVerdict:
    """Authoritative normalization function returning CanonicalVerdict enum."""
    if isinstance(value, CanonicalVerdict):
        return value
    if value is None:
        return CanonicalVerdict.UNKNOWN
    return CanonicalVerdict.from_str(str(value))

@dataclass
class LabelingConfig:
    MALICIOUS_THRESHOLD: int = 100  # Deprecated: A heuristic score alone never decides MALICIOUS; verdicts require authoritative intelligence.
    HEURISTIC_REVIEW_THRESHOLD: int = 35  # Supporting evidence threshold for analyst prioritization (NOT a verdict decider)
    REVIEW_THRESHOLD: int = 35  # Deprecated alias for backwards compatibility
    SUSPICIOUS_THRESHOLD: int = 35  # Deprecated alias for backwards compatibility
