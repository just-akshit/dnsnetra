...
class AlienVaultOTXProvider(BaseThreatProvider):
    ...
    def lookup(self, domain: str) -> ThreatProviderResult:
        ...
        for attempt in range(max_attempts):
            ...
            try:
                retryable = {500, 502, 503, 504} if self._key_manager else {429, 500, 502, 503, 504}
                resp = self._request_with_retry(
                    "GET", general_url, headers=headers, retryable_statuses=retryable
                )
        ...
        for _analysis_attempt in range(max_analysis):
            ...
            try:
                ...
                analysis_resp = self._request_with_retry(
                    "GET", analysis_url, headers=analysis_headers, retryable_statuses=retryable
                )
            except requests.exceptions.RequestException as exc:
                logger.warning(
