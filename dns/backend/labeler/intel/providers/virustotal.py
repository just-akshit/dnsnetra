...
class VirusTotalProvider(BaseThreatProvider):
    ...
    def lookup(self, domain: str) -> ThreatProviderResult:
        ...
        has_positive_signal = (malicious > 0 or harmless > 0 or suspicious > 0)

        if is_malicious:
            confidence = malicious / total
            logger.info("[VT] Malicious — %s (confidence=%.4f)", domain, confidence)
        elif harmless > 0:
            confidence = harmless / total
            logger.info("[VT] Clean (Confirmed Harmless) — %s (confidence=%.4f)", domain, confidence)
        elif suspicious > 0:
            confidence = suspicious / total
            logger.info("[VT] Suspicious — %s (confidence=%.4f)", domain, confidence)
        else:
            confidence = 0.0
            logger.info("[VT] Unrated / Undetected (No Positive Signals) — %s", domain)

        logger.info(
            "[VT] Lookup completed — %s: malicious=%d harmless=%d suspicious=%d undetected=%d total=%d found=%s",
            domain, malicious, harmless, suspicious, undetected, total, has_positive_signal,
        )
