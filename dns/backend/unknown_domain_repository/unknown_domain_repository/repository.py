...
class UnknownDomainRepository:
    ...
    def insert_domain(
        ...
        try:
            with self.db_manager.transaction() as conn:
                ...
                # Create updated domain from database-generated row
                updated_domain = UnknownDomain.from_database_row(result)
                
                log_domain_operation("insert", domain.domain, success=True, extra={'id': updated_domain.id})
            ...
            with self.db_manager.transaction() as conn:
                ...
                if update_last_seen:
                    conflict_update = sql.SQL("""
                        DO UPDATE SET
                            ...
                            first_seen = LEAST(excluded.first_seen, {table}.first_seen),
                            query_count = {table}.query_count + 1,
                            updated_at = NOW(),
                            metadata = jsonb_set(
                                COALESCE({table}.metadata, excluded.metadata, jsonb_build_object()),
                                ARRAY['query_count'],
                                to_jsonb({table}.query_count + 1)
                            )
                    """).format(
                        table=sql.Identifier(DOMAIN_TABLE_NAME),
                    )
                ...
                if result:
                    ...
                    if isinstance(result, dict) or hasattr(result, "get"):
                        was_inserted = bool(result.get("was_inserted", False))
                        updated_domain = UnknownDomain.from_database_row(result)
                    else:
                        was_inserted = bool(result[-1])
                        updated_domain = UnknownDomain.from_database_row(result[:-1])
                    
                    operation = "insert" if was_inserted else "update"
                    log_domain_operation(f"upsert_{operation}", domain.domain, success=True, extra={'id': updated_domain.id})
    ...
    def update_external_review(
        ...
        """
        Record the result of an external intelligence check (VT / OTX) or recheck.
        Sets:
            previous_status = status
            status = new_status
            last_checked = review_time
            updated_at = NOW()
        """
        try:
            normalized_domain = validate_domain_name(domain_name)
            rev_time = review_time or datetime.now(timezone.utc)
            status_val = new_status.value if isinstance(new_status, DomainStatus) else str(new_status)
                ...
                if metadata is not None:
                    ...
                    query = sql.SQL("""
                        ...
                        SET {} = {},
                            {} = %s,
                            {} = %s,
                            {} = jsonb_set(COALESCE({}, '{{}}'::jsonb), ARRAY['last_review'], %s::jsonb),
                            {} = NOW()
                        WHERE {} = %s
                    """).format(
                        sql.Identifier(DOMAIN_TABLE_NAME),
                        sql.Identifier(COLUMN_PREVIOUS_STATUS),
                        sql.Identifier(COLUMN_STATUS),
                        sql.Identifier(COLUMN_STATUS),
                        sql.Identifier(COLUMN_LAST_CHECKED),
                        sql.Identifier(COLUMN_METADATA),
                        sql.Identifier(COLUMN_METADATA),
                        sql.Identifier(COLUMN_UPDATED_AT),
                        sql.Identifier(COLUMN_DOMAIN),
                    )
                    conn.execute(query, (status_val, rev_time, metadata_json, normalized_domain))
                else:
                    query = sql.SQL("""
                        UPDATE {}
                        SET {} = {},
                    ...
                    conn.execute(query, (status_val, rev_time, normalized_domain))

            log_domain_operation("external_review", normalized_domain, success=True, extra={"status": status_val})
            return True
        except Exception as e:
            log_domain_operation("external_review", domain_name, success=False, extra={"error": str(e)})
            raise RepositoryError(f"Failed to update external review for {domain_name}: {e}") from e

    def record_query_observation(
        self,
        domain_name: str,
        observation_time: datetime,
    ) -> bool:
        """
        Lightweight atomic update for repeated DNS queries.
        Increments query_count, updates last_seen = GREATEST(last_seen, observation_time),
        and first_seen = LEAST(first_seen, observation_time), without modifying status, previous_status, or last_checked.
        """
        try:
            normalized_domain = validate_domain_name(domain_name)
            obs_time = observation_time if observation_time.tzinfo else observation_time.replace(tzinfo=timezone.utc)
            ...
            with self.db_manager.transaction() as conn:
                ...
                """).format(
                    sql.Identifier(DOMAIN_TABLE_NAME),
                    sql.Identifier(COLUMN_LAST_SEEN),
                    sql.Identifier(COLUMN_LAST_SEEN),
                    sql.Identifier(COLUMN_FIRST_SEEN),
                    sql.Identifier(COLUMN_FIRST_SEEN),
                    sql.Identifier(COLUMN_QUERY_COUNT),
                    sql.Identifier(COLUMN_QUERY_COUNT),
                    sql.Identifier(COLUMN_UPDATED_AT),
                    sql.Identifier(COLUMN_DOMAIN),
                )
                conn.execute(query, (obs_time, obs_time, normalized_domain))

            return True
        except Exception as e:
            raise RepositoryError(f"Failed to record query observation for {domain_name}: {e}") from e

    def get_domains_for_recheck(
        self,
        batch_size: int = 100,
        older_than_hours: int = 24,
        eligible_statuses: Optional[Tuple[str, ...]] = ("clean", "review_needed", "new"),
    ) -> List[UnknownDomain]:
        """
        Find domains eligible for periodic intelligence recheck.
        Matches domains with status in eligible_statuses (default: 'clean', 'review_needed', 'new')
        where last_checked IS NULL OR last_checked < NOW() - INTERVAL '%s hours'.
        """
        try:
            target_statuses = tuple(
                s.value if isinstance(s, DomainStatus) else str(s).lower()
                for s in (eligible_statuses or ("clean", "review_needed", "new"))
            )
            query = sql.SQL("""
                SELECT * FROM {}
                WHERE status::text IN ({})
                  AND (last_checked IS NULL OR last_checked < NOW() - (%s * INTERVAL '1 hour'))
                ORDER BY COALESCE(last_checked, first_seen) ASC
                LIMIT %s;
            """).format(
                sql.Identifier(DOMAIN_TABLE_NAME),
                sql.SQL(", ").join(sql.Literal(st) for st in target_statuses),
            )

            with self.db_manager.get_connection() as conn:
                conn.row_factory = dict_row
                rows = conn.execute(query, (older_than_hours, batch_size)).fetchall()
                return [UnknownDomain.from_database_row(r) for r in rows]
        except Exception as e:
            raise RepositoryError(f"Failed to fetch domains for recheck: {e}") from e
            ...
            if update_last_seen:
                conflict_action_sql = sql.SQL("""
                    ({domain_col}) DO UPDATE SET
                        ...
                        {first_seen_col} = LEAST(excluded.{first_seen_col}, {table}.{first_seen_col}),
                        {query_count_col} = {table}.{query_count_col} + 1,
                        {updated_at_col} = NOW(),
                        {metadata_col} = jsonb_set(
                            COALESCE({table}.{metadata_col}, excluded.{metadata_col}, jsonb_build_object()),
                            ARRAY['query_count'],
                            to_jsonb({table}.{query_count_col} + 1)
                        )
                """).format(
                    domain_col=sql.Identifier(COLUMN_DOMAIN),
                    last_seen_col=sql.Identifier(COLUMN_LAST_SEEN),
                    first_seen_col=sql.Identifier(COLUMN_FIRST_SEEN),
                    query_count_col=sql.Identifier(COLUMN_QUERY_COUNT),
