...
class UnknownDomainService:
    ...
    def _process_domains_batch(
        ...
        for record in domains:
            ...
            rec_ts = record.get("query_timestamp") or record.get("timestamp")
            rec_obs = observed_at
            if rec_ts:
                try:
                    if isinstance(rec_ts, datetime):
                        rec_obs = rec_ts if rec_ts.tzinfo else rec_ts.replace(tzinfo=timezone.utc)
                    else:
                        dt = datetime.fromisoformat(str(rec_ts).replace("Z", "+00:00"))
                        rec_obs = dt if dt.tzinfo else dt.replace(tzinfo=timezone.utc)
                except Exception:
                    pass
            try:
    ...
    def update_external_review(
        ...
        """Record external intelligence check (VT/OTX) or recheck result."""
        if not self._repository:
            raise InitializationError("Service not initialized")
        return self._repository.update_external_review(domain_name, new_status, review_time, metadata)
    ...
    def record_query_observation(
        ...
        """Record repeat DNS query observation without modifying status or last_checked."""
        if not self._repository:
            raise InitializationError("Service not initialized")
        return self._repository.record_query_observation(domain_name, observation_time)
    ...
    def get_domains_for_recheck(
        ...
        """Fetch domains needing periodic recheck."""
        if not self._repository:
            raise InitializationError("Service not initialized")
        return self._repository.get_domains_for_recheck(batch_size, older_than_hours, eligible_statuses)
