...
const TimeSeriesChartCore = memo(function TimeSeriesChartCore({
  ...
  const dateLabels = useMemo(() => {
    ...
    // Check if dataset spans within 36 hours or has timeStr / time fields
    const firstDate = xAccessor(visiblePlotData[0]);
    const lastDate = xAccessor(visiblePlotData[visiblePlotData.length - 1]);
    const timeSpanMs = Math.abs((lastDate?.getTime?.() ?? 0) - (firstDate?.getTime?.() ?? 0));
    const isIntraday =
      timeSpanMs <= 36 * 3600 * 1000 ||
      visiblePlotData.some((d) => typeof d.timeStr === "string" || typeof d.time === "string");

    return visiblePlotData.map((d) => {
      if (typeof d.timeStr === "string" && d.timeStr) return d.timeStr;
      if (typeof d.time === "string" && d.time && /^\d{1,2}:\d{2}/.test(d.time)) return d.time;

      const date = xAccessor(d);
      if (isIntraday && date instanceof Date && !isNaN(date.getTime())) {
