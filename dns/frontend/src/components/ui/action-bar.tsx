...
function ActionBar(props: ActionBarProps) {
  ...
  React.useEffect(() => {
    propsRef.current = { onEscapeKeyDown, onOpenChange };
  });
    ...
    <ActionBarContext.Provider value={contextValue}>
      {ReactDOM.createPortal(
        <RootPrimitive
          ...
          ref={composedRef}
          className={cn(
            "fixed z-50 rounded-lg border border-border/80 bg-card/95 backdrop-blur-md shadow-xl outline-none",
            ...
            orientation === "horizontal"
              ...
              : "flex flex-col items-start gap-2 px-2 py-3",
            className,
          )}
          style={{
            [side]: `${sideOffset}px`,
...
function ActionBarSelection(props: DivProps) {
  ...
  return (
    <SelectionPrimitive
      ...
      {...selectionProps}
      className={cn(
        "flex items-center gap-1.5 rounded-md border border-border/60 bg-muted/30 px-2.5 py-1 font-medium text-xs text-foreground tabular-nums",
        className,
      )}
    />
  );
...
function ActionBarItem(props: ActionBarItemProps) {
  ...
  return (
    <Button
      ...
      {...itemProps}
      className={cn(
        "h-7 text-xs font-medium cursor-pointer",
...
function ActionBarClose(props: ActionBarCloseProps) {
  ...
  return (
    <ClosePrimitive
      ...
      {...closeProps}
      className={cn(
        "rounded-md p-1 opacity-70 outline-none hover:opacity-100 hover:bg-accent focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none cursor-pointer text-muted-foreground hover:text-foreground",
        className,
      )}
...
function ActionBarSeparator(props: ActionBarSeparatorProps) {
  ...
  return (
    <SeparatorPrimitive
      ...
      className={cn(
        "bg-border",
        orientation === "horizontal" ? "h-4 w-px mx-0.5" : "h-px w-full my-0.5",
        className,
      )}
    />
  );
