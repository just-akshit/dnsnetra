...
class ConnectionPoolManager:
    ...
    def initialize_pool(cls) -> None:
        ...
        try:
            ...
            # Enforce UTC session timezone at connection initialization
            if "options" in db_params:
                if "timezone=" not in db_params["options"]:
                    db_params["options"] = f"{db_params['options']} -c timezone=UTC"
            else:
                db_params["options"] = "-c timezone=UTC"
            ...
            logger.info(
                ...
                f"max_conn={max_conn}) for database '{db_params.get('database')}' with UTC timezone"
            )
    ...
    def get_connection(cls) -> Any:
        ...
        Enforces UTC session timezone on checkout.
        ...
        try:
            ...
            # Invariant: Analytical connections must operate in UTC session timezone
