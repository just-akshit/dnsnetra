    ...
    def process_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for domain, group in grouped:
            ...
            if 'final_label' in group.columns:
                canon_labels = group['final_label'].astype(str).map(lambda l: CanonicalVerdict.from_str(l).value)
                malicious_count = int((canon_labels == CanonicalVerdict.MALICIOUS.value).sum())
                clean_count = int((canon_labels == CanonicalVerdict.BENIGN.value).sum())
                review_needed_count = int((canon_labels == CanonicalVerdict.REVIEW_NEEDED.value).sum())
                unknown_count = int((canon_labels == CanonicalVerdict.UNKNOWN.value).sum())
            ...
            profiles_to_upsert.append({
                ...
                "review_needed_queries": review_needed_count,
