...
""",
    ...
    parser.add_argument(
        "--from-beginning",
        action="store_true",
        help="Consume Kafka topic from the earliest offset instead of latest",
    )
...
class LivePipelineProcessor:
    ...
    def _process_record(self, record: DNSRecord, line_number: int) -> None:
        ...
        try:
            ...
            threat_score, label, confidence, label_reason, ti_source = 0, "Benign", 50, "", "unknown"

        row.update({
            "threat_score": threat_score,
            "label":        label,
            "final_label":  CanonicalVerdict.from_str(label).value,
        ...
        #
        if record.domain:
            if ti_source in ("reputation", "malicious_feed", "malicious"):
                logger.info(
                    "TI lookup: %s | Source: %s | Result: MALICIOUS | Online TI: SKIPPED",
                    record.domain, ti_source,
                )
            elif ti_source in ("trusted", "reviewed_clean"):
                logger.info(
                    "TI lookup: %s | Source: %s | Result: CLEAN | Online TI: SKIPPED",
                    record.domain, ti_source,
                )
            elif ti_source == "daily_review":
                logger.info(
                    "TI lookup: %s | Source: %s | Result: REVIEW_NEEDED | Online TI: SKIPPED",
                    record.domain, ti_source,
                )
            elif ti_source == "online_ti":
                logger.info(
                    "TI lookup: %s | Source: %s | Result: %s (Investigated)",
                    record.domain, ti_source, label,
                )
            else:
                logger.info(
                    "TI lookup: %s | Source: %s | Result: %s",
                    record.domain, ti_source, label,
                )
...
def main() -> int:
    ...
    try:
        ...
        # ── Kafka consumer ───────────────────────────────────────────────────
        auto_offset = "earliest" if getattr(args, "from_beginning", False) else "latest"
        consumer_group = (
            f"dns_threat_pipeline_replay_{int(time.time())}"
            if getattr(args, "from_beginning", False)
            else "dns_threat_pipeline"
        )
